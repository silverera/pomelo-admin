...
MqttClient.prototype.connect = function(host, port, cb) {
	...
	this.socket.on('error', function(err) {
		logger.error('mqtt socket is error, remote server host: %s, port: %s, err:%s', host, port, err.toString());
	...
	if (this.lastPing > 0) {
		if (this.lastPong < this.lastPing) {
			if (now - this.lastPing > KEEP_ALIVE_TIMEOUT) {
				logger.error('%s mqtt rpc client checkKeepAlive error timeout for %d', this.id, KEEP_ALIVE_TIMEOUT);
				this.close();
			}
		} else {
			this.socket.pingreq();
